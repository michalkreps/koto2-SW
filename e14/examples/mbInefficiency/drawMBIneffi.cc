...
int main(int ,char** )
{
  ...
  char tit[256];
  E14FsimMBwithBCV300keVInefficiency mb;
  E14FsimMBProposalInefficiency mbprop;
  TGraphErrors* grES171=0;
  for(int i=0;i<5;i++) {
    double theta = vTheta[i];
    mbprop.drawFrame();

    TGraph* grP = mbprop.draw(theta);

    if(i==0) {
      grES171 = mbprop.drawES171();
    }
    grES171->Draw("P");


    TGraph* gr0 = mb.draw(0.5,theta);
    gr0->SetLineWidth(2);
    
    TGraph* gr1 = mb.draw(1.,theta);
    gr1->SetLineWidth(2);
    gr1->SetLineColor(2);
    
    TGraph* gr2 = mb.draw(2.,theta);
    gr2->SetLineColor(3);
    gr2->SetLineWidth(2);
    
    
    TGraph* gr3 = mb.draw(3.,theta);
    gr3->SetLineColor(4);
    gr3->SetLineWidth(2);
    ...
    std::sprintf(tit,"E391a MB (E_{th} = 0.5 MeV)");
    (leg.AddEntry(gr0,tit,"L"))->SetTextFont(22);

    std::sprintf(tit,"E391a MB (E_{th} = 1 MeV)");
    (leg.AddEntry(gr1,tit,"L"))->SetTextFont(22);

    std::sprintf(tit,"E391a MB (E_{th} = 2 MeV)");
    (leg.AddEntry(gr2,tit,"L"))->SetTextFont(22);

    std::sprintf(tit,"E391a MB (E_{th} = 3 MeV)");
