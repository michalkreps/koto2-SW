...
int main(int ,char** )
{
  ...
  for(int i=0;i<5;i++) {
    ...
    zsb.selectVersion(95);
    TGraph* gr0 = zsb.draw(1.,theta);
    gr0->SetLineWidth(2);

    zsb.selectVersion(105);
    TGraph* gr1 =zsb.draw(1.,theta);
    gr1->SetLineWidth(2);
    gr1->SetLineColor(2);
    ...
    std::sprintf(tit,"KOTO ZSB (E_{th} = 1 MeV,G4 9.5)");
    (leg.AddEntry(gr0,tit,"L"))->SetTextFont(22);

    std::sprintf(tit,"KOTO ZSB (E_{th} = 1 MeV,G4 10.5)");
