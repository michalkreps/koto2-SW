  ...
  //  thePionName3 should be "pi0"
{
  ...
  // decay mode is specified with parent and daughters
  // norm is defined decay mode by decay mode
  ...
  if ( (theParentName == "kaon0L") && 
    ...
    coeffH = 0.076;
    coeffAK = 0.0099;
    //2020.2.20 1.0342 was not large enough
    //norm = 1.0342;
    calculateNorm();
...
G4DecayProducts* GsimKPi3DecayChannel::DecayIt(G4double) 
{
  ...
  daughterM[0] = my_daughters[0]->GetPDGMass();
  daughterM[1] = my_daughters[1]->GetPDGMass();
  daughterM[2] = my_daughters[2]->GetPDGMass();
...
//     -2*massK*(daughterE[idPi1] + daughterM[idPi1]);
  ...
  daughterparticle = 
       new G4DynamicParticle( my_daughters[1], momentum1);
  products->PushProducts(daughterparticle);
...
void  GsimKPi3DecayChannel::calculateNorm()
{
#ifdef GSIMDEBUG
  GsimMessage::getInstance()->debugEnter(__PRETTY_FUNCTION__);
#endif
  ...
  if(m_modeFlag==0) {
    //Calculate maximum Dalitz weight
    G4ParticleTable* pTable = G4ParticleTable::GetParticleTable();
    G4ParticleDefinition* particle= pTable->FindParticle("kaon0L");
    particle= pTable->FindParticle("pi+");
    double massPi1 = particle->GetPDGMass();
    double PiPlusMass = massPi1;
    particle= pTable->FindParticle("pi-");
    double massPi2 = particle->GetPDGMass();
    particle= pTable->FindParticle("pi0");
    double massPi3 = particle->GetPDGMass();
    
    double Q=massK;
    double P2= -massPi1*massPi1 +  std::pow( (Q-massPi3)/2.,2.);
    double P=std::sqrt(P2);
    double Tpi1=std::sqrt(   std::pow(massPi1,2.) + std::pow(P,2.) ) -massPi1;
    double Tpi2=std::sqrt(   std::pow(massPi2,2.) + std::pow(P,2.) ) -massPi2;
    double Tpi3=0.;
    double PIN = 1./PiPlusMass /PiPlusMass;       
    double  S0=(massK*massK + massPi1*massPi1 + massPi2*massPi2 + massPi3*massPi3 )/3 ;
    double  S12=(Tpi1-Tpi2)*massK*2 ;
    double  S12SQ=S12*S12*PIN*PIN ;
    double  S3=(massK-massPi3)*(massK-massPi3) - 2.*massK*Tpi3 ;
    double  S30=(S3-S0)*PIN ;
    norm=(1. + coeffG *S30 + coeffH * S30*S30 + coeffAK * S12SQ)/2.0;
  } else if(m_modeFlag==1) {
    norm=0.5;
  } else {
    norm=0.5;
  }
