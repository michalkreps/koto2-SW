...
G4DecayProducts* GsimKL3DecayChannel::DecayIt(G4double) 
{
  ...
  //CheckAndFillParent is called in GetParent
  ...
  //   FillDaughters();
  //  }
  //CheckAndFillDaughters is called in GetDaughter
  G4ParticleDefinition* my_daughters[3];
  my_daughters[idPi] = GetDaughter(idPi);
  my_daughters[idLepton] = GetDaughter(idLepton);
  my_daughters[idNutrino] = GetDaughter(idNutrino);



  daughterM[idPi] = my_daughters[idPi]->GetPDGMass();
  daughterM[idLepton] = my_daughters[idLepton]->GetPDGMass();
  daughterM[idNutrino] = my_daughters[idNutrino]->GetPDGMass();
