...
double GsimE14dist2d_n::lagrange(double x[], double y[], double xp0, int n, int ord) {
...
void GsimE14dist2d_n::sampl2d(double& xpart, double& ypart, double& zpart, double& px, double& py, double& pz) {
  ...
  if (first) {
    ...
    ifstream fin;
    std::string ifname = 
      std::getenv("E14_TOP_DIR")+std::string("/share/GsimE14Spectrum/dist2d_C3_n.dat");
    ...
    for (int i=1;i<npt1;i++) {
      if (i<11) ptval[i] = i*0.01;
      else if (i==11) ptval[i] = 0.15;
      else if (i==12) ptval[i] = 0.2;
      else ptval[i] = (i-10)*0.1;
    }
    first = false;
#ifdef GSIMDEBUG
    fout.close();
...
GsimE14dist2d_n::GsimE14dist2d_n(string name) : GsimSpectrum(name) {
...
void GsimE14dist2d_n::generateSpecial(const G4Event* anEvent) {
