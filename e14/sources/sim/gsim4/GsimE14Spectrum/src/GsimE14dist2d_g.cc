...
double GsimE14dist2d_g::lagrange(double* x, double* y, double xp0, int n) {
  ...
  for (i = 0;i<n;i++) {
    mult = 1;
    for (j = 0;j<n;j++) {
      if (i!=j) mult*= (xp0-x[j])/(long double)(x[i]-x[j]);
    }
    out += mult*y[i];
  }
  return out;
...
double GsimE14dist2d_g::lagrange(double x[], double y[], double xp0, int n, int ord) {
...
void GsimE14dist2d_g::sampl2d(double& xpart, double& ypart, double& zpart, double& px, double& py, double& pz) {
  ...
  if (first) {
    ...
    ifstream fin;
    std::string ifname = 
      std::getenv("E14_TOP_DIR")+std::string("/share/GsimE14Spectrum/dist2d_C3_g.dat");
    ...
    for (int i=1;i<npt1;i++) {
      if (i<11) ptval[i] = i*0.001;
      else if (i==11) ptval[i] = 0.015;
      else if (i==12) ptval[i] = 0.02;
      else if (i<21) ptval[i] = (i-10)*0.01;
      else if (i==21) ptval[i] = 0.15;
      else if (i==22) ptval[i] = 0.2;
      else ptval[i] = (i-20)*0.1;
    }
    first = false;
#ifdef GSIMDEBUG
    fout.close();
...
GsimE14dist2d_g::GsimE14dist2d_g(string name) : GsimSpectrum(name) {

  GsimPersistencyManager::getPersistencyManager()
    ->createHistogram("distMomentum","Momentum [GeV/c]", 100,0,0.2);
  GsimPersistencyManager::getPersistencyManager()
    ->createHistogram("distX", "Initial x [cm]",100,-2,2);
  GsimPersistencyManager::getPersistencyManager()
    ->createHistogram("distY", "Initial y [cm]",100,-2,2);
  GsimPersistencyManager::getPersistencyManager()
    ->createHistogram("distpX", "Initial X momentum",100,0,0.0005);
  GsimPersistencyManager::getPersistencyManager()
    ->createHistogram("distpY", "Initial Y momentum", 100,0,0.0005);
  GsimPersistencyManager::getPersistencyManager()
    ->createHistogram("distpZ", "Initial Z momentum", 100,0,0.2);
  ...
  GsimPersistencyManager::getPersistencyManager()
    ->createHistogram("fDistP","File Distribution Momentum",40,0,40);
  ...
  GsimPersistencyManager::getPersistencyManager()
    ->createHistogram("fCDFP","File CDF Momentum",40,0,40);

  GsimPersistencyManager::getPersistencyManager()
    ->createHistogram("XInterp","Output of X Interpolation",200,0,2000);
  GsimPersistencyManager::getPersistencyManager()
    ->createHistogram("YInterp","Output of Y Interpolation",200,0,2000);
  GsimPersistencyManager::getPersistencyManager()
    ->createHistogram("PInterp","Output of P Interpolation",60,0,0.2);
}

void GsimE14dist2d_g::generateSpecial(const G4Event* anEvent) {
