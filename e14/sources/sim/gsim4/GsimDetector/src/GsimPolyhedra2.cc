...
(G4Polyhedra& hedra, const G4int , const G4VPhysicalVolume*) const
{
#ifdef GSIMDEBUG
  GsimMessage::getInstance()->debugEnter(__PRETTY_FUNCTION__);
#endif
...
void GsimPolyhedra2::extractTwoPlanesDXF(std::list<TVector3>& v0List,
					 ...
					 )
{
  ...
  double phi0  =m_parameterArray[0];
  double dphi  =m_parameterArray[1];
  double nSide =m_parameterArray[2];
  double dz    =m_parameterArray[3]/2.;
  double d0    =m_parameterArray[4];
  double d1    =m_parameterArray[5];

  double theta=dphi/nSide;
  double r0=d0/std::cos(theta/2.);
  double r1=d1/std::cos(theta/2.);
    
  for(int i=0;i<nSide;i++) {
    double phi=theta*i + phi0;
