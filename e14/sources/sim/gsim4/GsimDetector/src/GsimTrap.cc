...
(G4Trap& trap, const G4int , const G4VPhysicalVolume*) const
{
#ifdef GSIMDEBUG
  GsimMessage::getInstance()->debugEnter(__PRETTY_FUNCTION__);
#endif
...
void GsimTrap::extractTwoPlanesDXF(std::list<TVector3>& v0List,
				   ...
				   std::list<TVector3>& )
{
  ...
  double dz=m_parameterArray[0]/2.;
  double pTheta=m_parameterArray[1];
  double pPhi=m_parameterArray[2];
  double pDy1=m_parameterArray[3]/2.;
  double pDx1=m_parameterArray[4]/2.;
  double pDx2=m_parameterArray[5]/2.;
  double pAlp1=m_parameterArray[6];
  double pDy2=m_parameterArray[7]/2.;
  double pDx3=m_parameterArray[8]/2.;
  double pDx4=m_parameterArray[9]/2.;
