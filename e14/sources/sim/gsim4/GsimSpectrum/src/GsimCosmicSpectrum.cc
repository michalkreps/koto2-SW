...
GsimCosmicSpectrum::GsimCosmicSpectrum(std::string name)
  : GsimSpectrum(name)
{
#ifdef GSIMDEBUG
  ...
  delete messenger;
  messenger = new GsimCosmicSpectrumMessenger(this);
  ...
  // This function was obtained from the BESS experimental data
  // PLB 594(2004)35-46
  m_pMuonFunc = new TF1("m_pMuonFunc","pol6",0.6,16);
  m_pMuonFunc->SetParameters(34.721, -16.361, 3.6852,
			     -0.46829, 3.4014e-2, -1.3126e-3,
			     2.0826e-5);
  m_pMuonFunc->SetNpx(1000);
  // dN/d(Omega) is propotional to cos^2(theta),
  // which means that dN/d(theta) is propotional to sin(theta)*cos^2(theta)
  m_thetaFunc = new TF1("m_thetaFun","sin(x)*cos(x)*cos(x)",0.,TMath::Pi()/2.);
...
GsimCosmicSpectrum::~GsimCosmicSpectrum()
{
#ifdef GSIMDEBUG
  GsimMessage::getInstance()->debugEnter(__PRETTY_FUNCTION__);
#endif
...
void GsimCosmicSpectrum::generateSpecial(const G4Event* anEvent)
{
#ifdef GSIMDEBUG
  GsimMessage::getInstance()->debugEnter(__PRETTY_FUNCTION__);
#endif
  ...
  double p_muon = m_pMuonFunc->GetRandom() *GeV;
  double theta = m_thetaFunc->GetRandom();
