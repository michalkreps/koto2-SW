...
GsimE14Step2IsoeSpectrum::GsimE14Step2IsoeSpectrum(std::string name)
...
GsimE14Step2IsoeSpectrum::~GsimE14Step2IsoeSpectrum()
{
#ifdef GSIMDEBUG
  GsimMessage::getInstance()->debugEnter(__PRETTY_FUNCTION__);
#endif
...
void GsimE14Step2IsoeSpectrum::generateSpecial(const G4Event* anEvent)
{
#ifdef GSIMDEBUG
  GsimMessage::getInstance()->debugEnter(__PRETTY_FUNCTION__);
#endif
  ...
  if(m_beamCondition>=201504) {
    ...
    //Assume caloriemter z from target is (50+20)m
...
double GsimE14Step2IsoeSpectrum::pKLDistExit( double Pk )
{
  //30m from Isoe's  MT
...
double GsimE14Step2IsoeSpectrum::pKLDist(double* x,double* par)
{
  //x[0]: Pk with unit
  //par[0]: dZ with unit
