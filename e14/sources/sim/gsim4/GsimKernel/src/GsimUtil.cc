...
std::string GsimUtil::s_eBremsModel="SBM";
...
GsimUtil* GsimUtil::getUtil()
{
  ...
  s_eBremsModel="SBM";//G4SeltzerBergerModel for GsimQGSP_BERT
...
bool GsimUtil::getWorldHalfXYZ(double &halfX, double &halfY, double &halfZ)
{
#ifdef GSIMDEBUG
  GsimMessage::getInstance()->debugEnter(__PRETTY_FUNCTION__);
#endif
  ...
  if(pv) lv = pv->GetLogicalVolume();
  if(lv) sv = lv->GetSolid();
  if(!sv) {
    GsimMessage::getInstance()
      ->report("warning","No solidDetector is constructed for World");
    return false;
  }
  
  G4Box* box = dynamic_cast<G4Box*>(sv);
  if(!box) {
    GsimMessage::getInstance()
      ->report("warning","No G4Box for World.");
    return false;
  }
  
  halfX=box->GetXHalfLength();
  halfY=box->GetYHalfLength();
...
void GsimUtil::setBremsModel(std::string model)
{
#ifdef GSIMDEBUG
  GsimMessage::getInstance()->debugEnter(__PRETTY_FUNCTION__);
#endif
 ...
 std::string GsimUtil::getBremsModel()
{
#ifdef GSIMDEBUG
  GsimMessage::getInstance()->debugEnter(__PRETTY_FUNCTION__);
#endif
