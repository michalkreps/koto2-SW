...
//int GsimDetector::s_dxfCount=0;
//std::ofstream GsimDetector::s_dxfStream;
//bool GsimDetector::s_dxfFirstDump=false;
//std::list<std::string> GsimDetector::s_dxfLayerNameList;
...
void GsimDetector::setTranslationVector(G4ThreeVector vec)
{
  ...
  if(m_clonedPhysicalVolumeList.size()>0) {
    G4ThreeVector dTransV=vec-m_translationVector;
    std::list<G4ThreeVector>::iterator tvIt = m_clonedTranslationVectorList.begin();
    for(;tvIt!=m_clonedTranslationVectorList.end();tvIt++) {
      (*tvIt)=(*tvIt)+dTransV;
    }
  }
...
void GsimDetector::setRotationVector(G4ThreeVector vec)
{
  ...
  if(m_clonedPhysicalVolumeList.size()>0) {
    G4ThreeVector dRotV=vec-m_rotationVector;
    std::list<G4ThreeVector>::iterator rvIt = m_clonedRotationVectorList.begin();
    for(;rvIt!=m_clonedRotationVectorList.end();rvIt++) {
      (*rvIt)=(*rvIt)+dRotV;
    }
  }
...
void GsimDetector::setSensitiveDetector(std::string logicalVolumeName,std::string SDName,
					bool withHit, bool withDigi, bool withMtime,
					...
					GsimDigitizer* digi)
{
  ...
  setSensitiveDetector(logicalVolumeName,SDname,withHit,withDigi,withMtime,hitProcesser,digi);
...
void GsimDetector::setThisAndDaughterSensitiveDetector(std::string SDname,int channel,
						       bool withHit, bool withDigi, bool withMtime,
						       GsimDetector* hitProcesser,
						       GsimDigitizer* digi)
{
  ...
  setSensitiveDetector(SDname,channel,withHit, withDigi, withMtime,hitProcesser,digi);
  for(GsimDetectorContainer::iterator it=m_daughterDetectorContainer.begin();
      it!=m_daughterDetectorContainer.end();it++) {
    GsimDetector* daughter =  (*it).second;
    daughter->setThisAndDaughterSensitiveDetector(SDname,channel,withHit, withDigi, withMtime,hitProcesser,digi);
  }
...
void GsimDetector::setMtimeStore(std::string logicalVolumeName,bool withMtime)
{
#ifdef GSIMDEBUG
  GsimMessage::getInstance()->debugEnter(__PRETTY_FUNCTION__);
#endif
  for(std::map<G4LogicalVolume*,GsimSensitiveDetector*>::iterator
	it=m_sensitiveDetectorMap.begin();
      it!=m_sensitiveDetectorMap.end();it++) {
    if(std::string( ((*it).first)->GetName().data())==logicalVolumeName) {
      ((*it).second)->storeMtime(withMtime);
      m_isModified=true;
      return;
    }
  }
...
void GsimDetector::setMtimeStore(bool withMtime)
{
#ifdef GSIMDEBUG
  GsimMessage::getInstance()->debugEnter(__PRETTY_FUNCTION__);
#endif
  for(std::map<G4LogicalVolume*,GsimSensitiveDetector*>::iterator
    ...
    G4String tmpname = ((*it).first)->GetName();
    std::string logicalVolumeName=std::string(tmpname.data());
    setMtimeStore(logicalVolumeName,withMtime);
  }
#ifdef GSIMDEBUG
  GsimMessage::getInstance()->debugExit(__PRETTY_FUNCTION__);
#endif
...
void GsimDetector::setMtimeStoreDaughters(bool withMtime)
{
#ifdef GSIMDEBUG
  GsimMessage::getInstance()->debugEnter(__PRETTY_FUNCTION__);
#endif
  for(GsimDetectorContainer::iterator it=m_daughterDetectorContainer.begin();
      it!=m_daughterDetectorContainer.end();it++) {
    GsimDetector* daughter =  (*it).second;
    daughter->setMtimeStore(withMtime);
    daughter->setMtimeStoreDaughters(withMtime);
  }
#ifdef GSIMDEBUG
  GsimMessage::getInstance()->debugExit(__PRETTY_FUNCTION__);
#endif
...
void GsimDetector::setThisAndDaughterMtimeStore(bool withMtime)
{
#ifdef GSIMDEBUG
  GsimMessage::getInstance()->debugEnter(__PRETTY_FUNCTION__);
#endif
  setMtimeStore(withMtime);
  setMtimeStoreDaughters(withMtime);
#ifdef GSIMDEBUG
  GsimMessage::getInstance()->debugExit(__PRETTY_FUNCTION__);
#endif
}
...
void GsimDetector::comment()
{
#ifdef GSIMDEBUG
  GsimMessage::getInstance()->debugEnter(__PRETTY_FUNCTION__);
#endif

  std::cout << "GsimDetector has no comments" << std::endl;
...
void GsimDetector::setUserParameters(std::string, std::vector<double>) 
{
#ifdef GSIMDEBUG
  GsimMessage::getInstance()->debugEnter(__PRETTY_FUNCTION__);
#endif
...
void GsimDetector::update()
{
  ...
  if(m_userFlag==-999) {
    ...
    G4cout << m_className << " UserFlag:-999 only for comment. No detector construction." << G4endl;
  ...
  case 6:
    {
      ...
      if(! (theTrack->GetTrackStatus()==fAlive ||
	    theTrack->GetTrackStatus()==fSuspend)
	 )  return true;
...
bool GsimDetector::processHits(const G4Step* ,G4TouchableHistory*, std::list<int>&)
{
#ifdef GSIMDEBUG
  GsimMessage::getInstance()->debugEnter(__PRETTY_FUNCTION__);
#endif
...
void GsimDetector::dumpDaughters()
{
#ifdef GSIMDEBUG
  GsimMessage::getInstance()->debugEnter(__PRETTY_FUNCTION__);
#endif
  for(GsimDetectorContainer::iterator it=m_daughterDetectorContainer.begin();
      it!=m_daughterDetectorContainer.end();it++) {
    GsimDetector* daughter =  (*it).second;
    std::cout << daughter->getFullName() << std::endl;
    daughter->dumpDaughters();
  }
#ifdef GSIMDEBUG
  GsimMessage::getInstance()->debugExit(__PRETTY_FUNCTION__);
#endif
...
GsimDXFVolume* GsimDetector::registerDXFVolume()
{
#ifdef GSIMDEBUG
  GsimMessage::getInstance()->debugEnter(__PRETTY_FUNCTION__);
#endif
  ...
  std::list<TVector3> v0ListForThis;
  std::list<TVector3> v1ListForThis;
  std::list<TVector3> v2ListForThis;
  std::list<TVector3> v3ListForThis;
  extractTwoPlanesDXF(v0ListForThis,v1ListForThis,
		      v2ListForThis,v3ListForThis
		      );

  
  GsimDXFVolume* dxfVol = new GsimDXFVolume();
  if(m_isOuterVisible) {
    dxfVol->setSurfaceLoop0(v0ListForThis);
    dxfVol->setSurfaceLoop1(v1ListForThis);
    dxfVol->setSurfaceLoop2(v2ListForThis);
    dxfVol->setSurfaceLoop3(v3ListForThis);
  }
  
  dxfVol->setFullName( getFullName() );
  dxfVol->addTraV(
		  TVector3(m_translationVector.x(),
			   m_translationVector.y(),
			   m_translationVector.z())
		  );
  dxfVol->addRotV(
		  TVector3(m_rotationVector.x(),
			   m_rotationVector.y(),
			   m_rotationVector.z())
		  );
  ...
  std::list<G4ThreeVector>::iterator tvIt = m_clonedTranslationVectorList.begin();
  std::list<G4ThreeVector>::iterator rvIt = m_clonedRotationVectorList.begin();
  for(;tvIt!=m_clonedTranslationVectorList.end();tvIt++,rvIt++) {
    dxfVol->addTraV(
		    TVector3((*tvIt).x(),
			     (*tvIt).y(),
			     (*tvIt).z())
  ...
  for(GsimDetectorContainer::iterator it=m_daughterDetectorContainer.begin();
      it!=m_daughterDetectorContainer.end();it++) {
    GsimDetector* daughter =  (*it).second;
    dxfVol->registerDaughter(
			     daughter->registerDXFVolume()
			     );
  }
...
void GsimDetector::extractTwoPlanesDXF(std::list<TVector3>& ,
				       std::list<TVector3>& ,
				       std::list<TVector3>& ,
				       std::list<TVector3>&
				       )
