...
#include "GsimKernel/GsimVersion.h"
...
void GsimEventAction::BeginOfEventAction(const G4Event* evt)
{
  ...
  if(eventID==0) {
    ...
    int gsimRelease=GSIM_RELEASE;
    int gsimVersion=GSIM_VERSION;
    int gsimCommit =GSIM_COMMIT;
...
void GsimEventAction::EndOfEventAction(const G4Event* evt)
{
  ...
  //force to discard the data if event is aborted.
  if(s_isEventAborted) {
    withDataToStore=false;
  }
