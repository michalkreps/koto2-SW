...
void GsimTrackingAction::PreUserTrackingAction(const G4Track* aTrack)
{
  ...
  if(aTrack->GetCurrentStepNumber()==0) {
    //PreUserTrackingAction is called serveral times
    // for the same track in some cases.
    //Step number is used to prevent overwriting the initial information.
...
void GsimTrackingAction::dumpPreTrack(const G4Track* aTrack)
{
  ...
  if(aTrack->GetCreatorProcess()) {
    G4cout << std::setw(30) << std::setiosflags(std::ios::left)
