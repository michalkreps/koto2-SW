...
protected:
  ...
  G4UIcmdWithAString*          m_cmdSetBremsModel;
};
  ...
  :m_factory(factory)
{
  ...
  m_cmdSetBremsModel = 0;
  if(factoryName=="GsimQGSP_BERT")
  {
    std::string commandName=dirName;
    commandName+="setBremsModel";
    m_cmdSetBremsModel = new G4UIcmdWithAString(G4String(commandName.c_str()),this);
    m_cmdSetBremsModel->SetGuidance("Set Bremsstrahlung model for GsimQGSP_BERT.");
    m_cmdSetBremsModel->SetGuidance("setBremsModel SBM / eBrems");
    m_cmdSetBremsModel->SetGuidance("SBM:G4SeltzerBergerModel:the current default model. (default)");
    m_cmdSetBremsModel->SetGuidance("eBrems:G4eBremsstrahlungModel:geant4.9.4, obsolete.");
    m_cmdSetBremsModel->AvailableForStates(G4State_PreInit,G4State_Idle);
  }
						     ...
						     G4String aValue)
{
  ...
  else if(command == m_cmdSetBremsModel)
    {
      if(aValue=="eBrems" || aValue=="SBM") {
	GsimUtil* util = GsimUtil::getUtil();
	util->setBremsModel(std::string(aValue.data()));
      } else {
	std::string msg = "Wrong argument at GsimPhysicListFactoryMessenger::";
