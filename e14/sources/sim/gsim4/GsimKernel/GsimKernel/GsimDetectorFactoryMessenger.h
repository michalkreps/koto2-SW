...
protected:
  ...
  /// A user Interface command under the directory.
  G4UIcmdWithoutParameter* m_cmdComment;
  ...
  :m_factory(factory)
{
  ...
  std::string commandPrintName=dirName;
  commandPrintName+="comment";
  m_cmdComment = new G4UIcmdWithoutParameter(G4String(commandPrintName.c_str()),this);
  m_cmdComment->SetGuidance("Print detector comment");
  m_cmdComment->AvailableForStates(G4State_PreInit,G4State_Idle);
