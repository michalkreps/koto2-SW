...
public:
  ...
  /**
   *  @function getWorldHalfXYZ
   *  @brief Get the world half dimensions
   *  @return true if succeeded
   */
  bool getWorldHalfXYZ(double &halfX, double &halfY, double &halfZ);
  ...
  /// set eBremsstrahlung model
  /**
   * This can be called before the 1st run in PreInit state.
   */
  void setBremsModel(std::string model);
  std::string getBremsModel();
 ...
 private:
  ...
  static std::string s_eBremsModel;
  /**
   * Bremsstrahlung model for GsimQGSP_BERT : SBM / eBrems
   */
};
