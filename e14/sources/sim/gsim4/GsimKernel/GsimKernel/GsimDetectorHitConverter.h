...
class GsimDetectorHitConverter
{
 public:
  ...
  /**
   *  @function fillHitData
   *  @brief fill GsimDetectorHit from G4Step
   *  @param data A pointer of GsimDetectorHitDatqa.
   *  @param aStep A pointer of G4Step.
   *  @param channelID hit channel ID. 
   */
  void fillHitData(GsimDetectorHitData* data,int thisID,
		   const G4Step* aStep,int channelID);
  
  /**
   *  @function convertHit
   *  @brief convert G4Step to GsimDetectorHit.
   *  @param eventData A pointer of GsimDetectorEventData.
   *  @param aStep A pointer of G4Step.
   *  @param ROhist A readout history.
   *  @param hitChannelList A list of hit channel IDs.
