...
public:
  ...
  /// Interface to print flag comment
  void comment();
...
template<class T> void GsimDetectorFactory<T>::comment()
{
#ifdef GSIMDEBUG
  {
    std::ostringstream ostr;
    ostr << "enter GsimDetectorFactory::comment()";
    GsimMessage::getInstance()
      ->report("debug",ostr.str());
  }
#endif

  T tmpDetector("tmpDetector",0,G4ThreeVector(0,0,0),G4ThreeVector(0,0,0),-999);
  tmpDetector.comment();
  ...
  {
    std::ostringstream ostr;
    ostr << "exit GsimDetectorFactory::comment()";
    GsimMessage::getInstance()
      ->report("debug",ostr.str());
  }
#endif
