...
bool GsimHEPEvtInterface::readOneAsciiLine()
{
  ...
  if(ISTHEP>0) {
    if(m_fileFormat==2) {
      aVertex = new G4PrimaryVertex(G4ThreeVector(VHEP1*mm +m_beamShiftX,
						  VHEP2*mm +m_beamShiftY,
						  VHEP3*mm -m_detectorShiftZ),
				    VHEP4*mm/c_light);
    }
  }
...
void GsimHEPEvtInterface::setDetectorShiftZ(double dZ)
{
  m_detectorShiftZ=dZ;
}

void GsimHEPEvtInterface::setBeamShiftX(double dX)
{
  m_beamShiftX=dX;
}

void GsimHEPEvtInterface::setBeamShiftY(double dY)
{
  m_beamShiftY=dY;
}

void GsimHEPEvtInterface::setLoopMode(bool isLoop)
{
  m_loopMode=isLoop;
}
...
bool GsimHEPEvtInterface::readOneRootEvent()
{
  ...
  if(ientry<0) {
    if(m_processed && m_loopMode) {
      m_eventCounter=0;
      ientry = tr->LoadTree(m_eventCounter);
    } else {
      return false;
    }
  }
