  ...
  :m_hepEvtInterface(interface)
{
...

  {
    ...
    m_cmdSetDetectorShiftZ->SetGuidance("set detector shift in Z");
    m_cmdSetDetectorShiftZ->AvailableForStates(G4State_PreInit,G4State_Idle);
  }

  {
    std::string commandName=dirName;
    commandName+="setBeamShiftXY";
    m_cmdSetBeamShiftXY = new G4UIcmdWithAString(G4String(commandName.c_str()),this);
    m_cmdSetBeamShiftXY->SetGuidance("set beam shift in X and Y");
    m_cmdSetBeamShiftXY->AvailableForStates(G4State_PreInit,G4State_Idle);
  }

  {
    std::string commandName=dirName;
    commandName+="setLoopMode";
    m_cmdSetLoopMode = new G4UIcmdWithABool(G4String(commandName.c_str()),this);
    m_cmdSetLoopMode->SetGuidance("set loop mode of seed file.");
    m_cmdSetLoopMode->SetGuidance("ture: HEPEvt file is rewined when # of events exceeds the entries.");
    m_cmdSetLoopMode->AvailableForStates(G4State_PreInit,G4State_Idle);
  }
...
void GsimHEPEvtInterfaceMessenger::SetNewValue(G4UIcommand* command,G4String newValue)
{
  ...
  if( command == m_cmdSetInputFileName )
    ...
    {
      GsimUtil* util = GsimUtil::getUtil();
      m_hepEvtInterface->setDetectorShiftZ(util->interpret(newValue));
    }
  else if(command == m_cmdSetBeamShiftXY)
    {
      GsimUtil* util = GsimUtil::getUtil();
      G4String tmpline = newValue;
      G4String tmpout;
	...
	if(ok) {
      ...
      if(vec.size()!=2) {
	std::string msg = "Wrong number of paramaeters setBeamShiftXY ";
	msg+="at GsimHEPEvtInterfaceMessenger::";
	msg+="SetNewValue(G4UIcommand* command,G4String newValue=";
	msg+=std::string(newValue.data()) ;
	msg+=")";
	...
	return;
      ...
      m_hepEvtInterface->setBeamShiftX(vec[0]);
      m_hepEvtInterface->setBeamShiftY(vec[1]);
    }
  else if(command == m_cmdSetLoopMode)
    {
      m_hepEvtInterface->setLoopMode(m_cmdSetLoopMode->GetNewBoolValue(newValue));
    }
  else {
  }
#ifdef GSIMDEBUG
