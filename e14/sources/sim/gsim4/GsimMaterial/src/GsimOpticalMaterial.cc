  ...
  : GsimMaterial()
{
  ...
  if(!mat) {
    ...
    opFP =
      new G4Material(name, density, nel=3);
    opFP->AddElement(nist->FindOrBuildElement("C"), 5);
    opFP->AddElement(nist->FindOrBuildElement("O"), 1);
    opFP->AddElement(nist->FindOrBuildElement("H"), 8);
  }
  ...
  if(!mat) {
    ...
    opPS =
      new G4Material(name, density, nel=2);
    opPS->AddElement(nist->FindOrBuildElement("C"), 8);
    opPS->AddElement(nist->FindOrBuildElement("H"), 8);
  }
  ...
  if(BC400) {
    ...
    G4double PhotonWaveLength[nEntries] = {
      1000.0,
      640.0, 630.0, 620.0, 610.0, 600.0,
      590.0, 580.0, 570.0, 560.0, 550.0,
      540.0, 530.0, 520.0, 510.0, 500.0,
      499.7, 494.2, 490.1, 485.9, 480.4,
      473.2, 466.9, 462.2, 458.6, 455.2,
      452.5, 451.1, 450.0, 448.1, 445.6,
      442.5, 439.8, 437.3, 435.1, 433.1,
      431.8, 430.4, 428.7, 427.3, 426.2,
      425.4, 424.6, 423.5, 421.5, 421.0,
      418.8, 416.6, 414.9, 413.8, 413.0,
      411.9, 409.4, 405.8, 403.3, 400.3,
      390.0, 380.0, 370.0, 360.0, 350.0,
      100.0
    };//[nm]
    ...
    G4double Scintillation[nEntries] = {
       0.000 ,
       0.000 ,  0.129 ,  0.180 ,  0.251 ,  0.349 ,
       0.487 ,  0.679 ,  0.946 ,  1.318 ,  1.837 ,
       2.561 ,  3.569 ,  4.974 ,  6.931 ,  9.660 ,
      10.526 , 12.465 , 13.850 , 15.789 , 18.559 ,
      23.269 , 28.255 , 32.964 , 37.119 , 42.382 ,
      47.091 , 50.416 , 55.125 , 59.003 , 64.543 ,
      68.698 , 70.360 , 70.914 , 71.191 , 72.853 ,
      75.069 , 80.332 , 85.596 , 91.413 , 96.953 ,
      98.892 , 99.446 , 99.723 , 98.237 , 96.122 ,
      90.305 , 81.440 , 70.637 , 54.571 , 43.767 ,
      34.903 , 23.823 , 14.404 ,  9.418 ,  5.263 ,
       1.229 ,  0.266 ,  0.058 ,  0.012 ,  0.000 ,
       0.000
    };
    for(int i=0;i<nEntries;i++) {
  ...
  if(CsI){
    ...
    G4double PhotonWaveLength[nEntries] = {
      1000 , 800 , 630 ,
      616.149 , 603.727 , 598.758 , 593.789 , 575.155 ,
      557.764 , 536.646 , 516.77 , 498.136 , 480.745 ,
      455.9 , 440.994 , 417.391 , 400 , 386.336 ,
      368.944 , 361.491 , 351.553 , 346.584 , 339.13 ,
      332.92 , 326.708 , 321.739 , 318.013 , 314.286 ,
      308.075 , 304.669 , 301.863 , 298.137 , 294.41 ,
      291.925 , 283.229 , 275.777 , 269.565 , 264.596 ,
      262.112 , 257.143 , 248.447 , 239.751 , 230 ,
      220 , 200 , 150 , 100
    };//[nm]
    G4double PhotonEnergy[nEntries];
    for(int i=0;i<nEntries;i++) {
    ...
    G4double RefractiveIndex[nEntries] = {
      1.758   , 1.7668  , 1.78085 ,
      1.78208 , 1.78332 , 1.78386 , 1.78444 , 1.78698 ,
      1.79007 , 1.79499 , 1.8011  , 1.80763 , 1.81311 ,
      1.82157 , 1.82804 , 1.84088 , 1.85247 , 1.86292 ,
      1.87846 , 1.88624 , 1.898   , 1.90455 , 1.91492 ,
      1.92362 , 1.93194 , 1.93803 , 1.94214 , 1.94594 ,
      1.95223 , 1.956   , 1.95946 , 1.96472 , 1.97097 ,
      1.97582 , 1.99818 , 2.02579 , 2.05628 , 2.08638 ,
      2.10354 , 2.14242 , 2.22666 , 2.3342  , 2.48637 ,
      2.68155 , 3.2097  , 5.55591 , 9.84344
    };
    G4double Scintillation[nEntries] = {
      0. , 0. , 0.,
      0. , 0.00359112 , 0.00718089 , 0.0179516 , 0.0359018 ,
      0.0574431 , 0.0753946 , 0.0753946 , 0.0825742 , 0.0789844 ,
      0.0718035 , 0.064624 , 0.064624 , 0.0682137 , 0.0861653 ,
      0.125657 , 0.157969 , 0.226183 , 0.283624 , 0.391331 ,
      0.538528 , 0.678545 , 0.818562 , 0.919087 , 0.97294 ,
      0.998071 , 1 , 0.9873 , 0.958579 , 0.872414 ,
      0.746759 , 0.531347 , 0.333888 , 0.19746 , 0.118476 ,
      0.064624 , 0.0394929 , 0.0143618 , 0.00718089 , 0. ,
      0. , 0. , 0. , 0.
    };
    G4double AttenuationLength[nEntries];
    for(int i=0;i<nEntries;i++) {
  ...
  if(SiCookie){
    ...
    G4double RefractiveIndex[nEntries] = {
      1.44433,        1.44517,        1.44614,        1.44724,        1.44851,        
      1.44999,        1.45173,        1.45381,        1.45639,        1.45975,        
      1.46452,        1.46575,        1.46711,        1.46863,        1.47033,        
      1.47225,        1.47444,        1.47696,        1.47987,        1.48327,       
      1.48727,        1.49202,        1.4977,         1.50455,        1.51289,        
      1.5231,         1.53573,        1.55146,        1.57122,        1.59625,        
      1.62822,        10,             1,              1,              1
    };
    G4MaterialPropertiesTable* MPT = new G4MaterialPropertiesTable();
    MPT->AddProperty("RINDEX",       PhotonEnergy, RefractiveIndex,nEntries);
  ...
  if(opFP){
    ...
    G4double refractiveIndex[nEntries];
    for(int i=0;i<nEntries;i++) {
      refractiveIndex[i]=1.42;
    }
    G4MaterialPropertiesTable* mpt = new G4MaterialPropertiesTable();
    mpt->AddProperty("RINDEX",       photonEnergy, refractiveIndex,nEntries);
    opFP->SetMaterialPropertiesTable(mpt);
  }
  ...
  if(opPS){
	...
	std::sqrt(1+
		  1.4435*std::pow(x,2.)/(
					 std::pow(x,2.)-0.020216
					 )
		  );
    }
    G4MaterialPropertiesTable* mpt = new G4MaterialPropertiesTable();
    mpt->AddProperty("RINDEX",       photonEnergy, refractiveIndex,nEntries);
    opPS->SetMaterialPropertiesTable(mpt);
  }
