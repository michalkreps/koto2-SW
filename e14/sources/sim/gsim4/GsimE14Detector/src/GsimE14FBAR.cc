  ...
  : GsimDetector(name,motherDetector,transV,rotV,userFlag)
{
  ...
  if(
    ...
    GsimMessage::getInstance()
      ->report("warning",
	       "Invalid user flag is assigned. FBAR is not constructed.");
    return;
  }
  ...
  double  fbbb_rmax = fbbb_rmin + 1.2;//12mm thick
  // double  p_fbbb[] = { -360./16/2, 360, 16, 2,
  ...
  // Front Barrel Backbone Fin
  // 24mm thick box with adjacent pair
  // Radial length (102-12) mm from fbbb_rmax
  double fbbf_thick  = 2.4;//cm
  double fbbf_height = (102-12)*0.1;//cm

  paramVec.clear();
  paramVec.push_back(fbbf_thick*cm);
  paramVec.push_back(fbbf_height*cm);
  paramVec.push_back(fbar_len*cm);
  std::sprintf(nam,"FBBF");
  GsimBox *boxFBBF = 0;
  for(int i=0;i<16;i++) {
    
    double  x = 0;
    double  y = (fbbb_rmax + fbbf_height/2.);
    double  z = (c_fbbb_z-fbar_len/2.);
    
    CLHEP::Hep3Vector c( x,y,z );
    c.rotateZ( 2*M_PI*i/16 );
    
    if(i==0) {
      boxFBBF = new GsimBox(std::string(nam),this,
			    G4ThreeVector(c.x()*cm,c.y()*cm,c.z()*cm),
			    G4ThreeVector(0,0,0));
      boxFBBF->setOuterMaterial("G4_Fe");
      boxFBBF->setParameters(paramVec);
      addDaughter(boxFBBF);
      m_detList.push_back(boxFBBF);
    } else {
      boxFBBF->cloneDetector(G4ThreeVector(c.x()*cm,c.y()*cm,c.z()*cm),
			     G4ThreeVector(0.,0.,(i*1.)/16.*2.*M_PI)
			     );
    }
  }
    
  // Front Barrel Backbone Plate
  // 12mm thick 
  // fbbb_rmax->fbbb_ramx+fbbf_height
  // X_bottom :+12mm -> fbbb_rmax*tan(th)
  // X_top    :+12mm -> (fbbb_rmax+fbbf_height)*tan(th)
  // Y        :fbbf_height
  double fbbp_thick = 1.2;
  double fbbp_height=fbbf_height;
  double fbbp_bottom=fbbb_rmax*tan(2.*M_PI/32.)-1.2;
  double fbbp_top=(fbbb_rmax+fbbf_height)*tan(2.*M_PI/32.)-1.2;
  double fbbp_alpha=
    atan( (fbbp_top-fbbp_bottom)/2./fbbp_height );
  paramVec.clear();
  paramVec.push_back(fbbp_thick*cm);
  paramVec.push_back(0*rad);
  paramVec.push_back(0*rad);
  paramVec.push_back(fbbp_height*cm);
  paramVec.push_back(fbbp_bottom*cm);
  paramVec.push_back(fbbp_top*cm);
  paramVec.push_back(fbbp_alpha*rad);
  paramVec.push_back(fbbp_height*cm);
  paramVec.push_back(fbbp_bottom*cm);
  paramVec.push_back(fbbp_top*cm);
  paramVec.push_back(fbbp_alpha*rad);
  std::sprintf(nam,"FBBP");
  GsimTrap* trapFBBP=0;
  for(int k=0;k<6;k++) {
    for(int j=0;j<2;j++) {
	...
	double dZ=(19+(12+528)*k+12/2.)*0.1;
	
	double  x = (1.2+(fbbp_top+fbbp_bottom)/4.);
	double  y = (fbbb_rmax+fbbp_height/2.);
	double  z = (c_fbbb_z-fbar_len+dZ);
	
	if(j==1) x=-x;
	
	double zRot=2*M_PI*i/16;
	
	CLHEP::Hep3Vector c( x,y,z );
	c.rotateZ( zRot );
	
	if(i==0 && j==0 && k==0) {
	  trapFBBP = new GsimTrap(std::string(nam),this,
				  G4ThreeVector( c.x()*cm,c.y()*cm,c.z()*cm),
				  G4ThreeVector(0.,yRot,zRot)
				  );
	  trapFBBP->setOuterMaterial("G4_Fe");
	  trapFBBP->setParameters(paramVec);
	  addDaughter(trapFBBP);
	  m_detList.push_back(trapFBBP);
	} else {
	  trapFBBP->cloneDetector(G4ThreeVector(c.x()*cm,c.y()*cm,c.z()*cm),
				  G4ThreeVector(0.,yRot,zRot)
				  );
	}
      }
  ...
  for( int i=1;i<54;i++ ){
    ...
    //double  p_fbld[] = { fbld_bottom/2, fbld_height/2, fbld_len/2 };
  ...
  sd->setDeepCopyNoFactor(2);


  //FB inner support structure added in KOTO
  double  fbis_rmin = 530*mm/2.;
  double  fbis_rmax = 610*mm/2.;
  double  fbis_len  = 20*mm;
  double  fbis_dist = 175*mm; //from FB downstram end
  paramVec.clear();
  paramVec.push_back(( -360./16./2.)*deg);
  paramVec.push_back(360*deg);
  paramVec.push_back(16);
  paramVec.push_back(fbis_len);
  paramVec.push_back(fbis_rmin);
  paramVec.push_back(fbis_rmax);
  sprintf(nam,"FBIS");
  GsimPolyhedra2* fbis = new GsimPolyhedra2(std::string(nam),this,
					    G4ThreeVector(0,0,(+fbar_len/2)*cm-fbis_dist-fbis_len/2.),
					    G4ThreeVector(0,0,0));
  fbis->setOuterMaterial("G4_Fe");
  fbis->setParameters(paramVec);
  fbis->setOuterColor("gray70");
  addDaughter(fbis);
  m_detMoveList.push_back(fbis);


  //Vacuum front cylinder
  double cylinder_len=315;//cm
  paramVec.clear();
  paramVec.push_back(172/2.*cm);
  paramVec.push_back(176/2.*cm);
  paramVec.push_back(cylinder_len*cm);
  paramVec.push_back(0);
  paramVec.push_back(360*deg);
  GsimTube* frontCylinder = new GsimTube("frontCylinder",this,
					 G4ThreeVector(0,0,(+fbar_len/2.-cylinder_len/2.)*cm),
					 G4ThreeVector(0,0,0));

  frontCylinder->setParameters(paramVec);
  frontCylinder->setOuterMaterial("GsimSUS");
  frontCylinder->setOuterColor("gray70");
  addDaughter(frontCylinder);
  m_detMoveList.push_back(frontCylinder);

  //Vacuum front flange
  double flange_len=4;//cm
  paramVec.clear();
  paramVec.push_back(40*cm/2.);
  paramVec.push_back(94.75*cm);
  paramVec.push_back(4*cm);
  paramVec.push_back(0);
  paramVec.push_back(360*deg);
  GsimTube* frontFlange = new GsimTube("frontFlange",this,
				       G4ThreeVector(0,0,(+fbar_len/2.-cylinder_len-flange_len/2.)*cm),
				       G4ThreeVector(0,0,0));
  
  frontFlange->setParameters(paramVec);
  frontFlange->setOuterMaterial("GsimSUS");
  frontFlange->setOuterColor("blue");
  addDaughter(frontFlange);
  m_detMoveList.push_back(frontFlange);
