  ...
  : GsimDetector(name,motherDetector,transV,rotV,userFlag)
{
  ...
  if(
    ...
    GsimMessage::getInstance()
      ->report("warning",
	       "Invalid user flag is assigned. CBAR is not constructed.");
    return;
  }

  bool withOuterAdditionalLayers=false;
  ...
  paramVec.push_back(cbar_len_top*cm+1.2*mm);//HNDEBUG 1.2*mm to fix overlap
  paramVec.push_back(cbar_top*cm);
  paramVec.push_back(cbar_top*cm);
  paramVec.push_back(0*deg);
  paramVec.push_back(cbar_len_bot*cm+1.2*mm);//HNDEBUG 1.2*mm to fix overlap
  ...
  // Layer structure
  {
    ...
    for( int i=0;i<44;i++ ){
      ...
      //double  p_cbld[] = { cbld_bottom/2, cbld_len_bot/2, cbld_height/2 };
  ...
  //Vacuum tank
  paramVec.clear();
  paramVec.push_back(335/2.*cm);
  paramVec.push_back(350.2/2.*cm);
  paramVec.push_back(605*cm);
  paramVec.push_back(0);
  paramVec.push_back(360*deg);
  double zShiftTank=25*mm;
  GsimTube* middleTank = new GsimTube("middleTank",this,
				      G4ThreeVector(0,0,zShiftTank),
				      G4ThreeVector(0,0,0));
  

  middleTank->setParameters(paramVec);
  middleTank->setOuterMaterial("GsimSUS");
  middleTank->setOuterColor("blue");
  addDaughter(middleTank);
  m_detMoveList.push_back(middleTank);
...
void GsimE14CBAR::comment()
{
#ifdef GSIMDEBUG
  GsimMessage::getInstance()->debugEnter(__PRETTY_FUNCTION__);
#endif

  std::cout << "Z center is the center of CBAR." << std::endl;
  std::cout << "Place it at z=(134.8+550./2.)*cm" << std::endl;
