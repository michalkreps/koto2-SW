  ...
  : GsimDetector(name,motherDetector,transV,rotV,userFlag)
{
  ...
  for( int y=0;y<27;y++ ){
    for( int x=0;x<27;x++ ){
	  ...
	  {
	    double  alpha2 =( atan( (snds_top[cd]-snds_bot[cd])/2/snds_hi[cd] ) );
	    double  offset_h2 = snds_hi[cd]/2 - sand_hi[cd]/2 + 0.2;
	    double  offset_x2 = snds_bot[cd]/4 - sand_bot[cd]/4 + 0.2;
	    // double p_snds[]={ Csic_Len/2, 0, 0,
	    // 		      snds_hi[cd]/2,snds_bot[cd]/2,snds_top[cd]/2,alpha2,
	    // 		     snds_hi[cd]/2,snds_bot[cd]/2,snds_top[cd]/2,alpha2 };
	    ...
	    GsimTrap* sci = new GsimTrap(std::string(nam),trap,
					 G4ThreeVector(offset_x2*cm,offset_h2*cm,0),
					 G4ThreeVector());
		...
		double  th = 0.2;
		//double  p_sndl[]={ th/2, l/2, Csic_Len/2 };
		paramVec.clear();
		paramVec.push_back(th*cm);
		paramVec.push_back(l*cm);
		paramVec.push_back(Csic_Len*cm);
		
		double  offset_h3 = -snds_hi[cd]/2;
		double  offset_x3 = -snds_bot[cd]/4;
		double  cx = bb - th/2 + offset_x3;
		double  cy = l/2  + offset_h3;
