...
// ********************************************************************
//
// $Id: GsimeBremsstrahlung.cc 107058 2017-11-01 14:54:12Z gcosmo $
//
// -------------------------------------------------------------------
//
// GEANT4 Class file
...
GsimeBremsstrahlung::GsimeBremsstrahlung(const G4String& name):
...
GsimeBremsstrahlung::~GsimeBremsstrahlung()
{}

//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo....

G4bool GsimeBremsstrahlung::IsApplicable(const G4ParticleDefinition& p)
{
...
void 
GsimeBremsstrahlung::InitialiseEnergyLossProcess(const G4ParticleDefinition*,
					       const G4ParticleDefinition*)
{
  if(!isInitialised) {
    ...
    energyLimit = std::min(emax, GeV);
    fm = 0;
#endif


    std::string model=GsimUtil::getUtil()->getBremsModel();
    if(model == "SBM") {
      m_mode=0;
    } else if( model == "eBrems") {
      m_mode=1;
    } else {
      m_mode=0;
      std::string msg = "Wrong brems model name, ";
      msg+=model;
      msg+=". SBM is used.";
    ...
    if(m_mode==0) {
      ...
      if (!EmModel(1)) { SetEmModel(new GsimeBremsstrahlungModel(),1); }
      EmModel(1)->SetLowEnergyLimit(emin);
      EmModel(1)->SetHighEnergyLimit(energyLimit);
      EmModel(1)->SetSecondaryThreshold(param->BremsstrahlungTh());
      EmModel(1)->SetLPMFlag(false);
...
void GsimeBremsstrahlung::StreamProcessInfo(std::ostream& outFile) const
{
  if(EmModel(0)) {
    G4EmParameters* param = G4EmParameters::Instance();
    G4double eth = param->BremsstrahlungTh(); 
    outFile << "      LPM flag: " << param->LPM() << " for E > " 
	    << EmModel(0)->HighEnergyLimit()/GeV << " GeV";
    if(eth < DBL_MAX) { 
      outFile << ",  VertexHighEnergyTh(GeV)= " << eth/GeV; 
    }
    outFile << G4endl;
  }
}

void GsimeBremsstrahlung::ProcessDescription(std::ostream& out) const
{
...
void GsimeBremsstrahlung::StreamProcessInfo(std::ostream& out,
...
#if ( G4VERSION_NUMBER/10 < 104 )
void GsimeBremsstrahlung::PrintInfo()
{
  if(EmModel(1)) {
#if ( G4VERSION_NUMBER/10 >= 101 )
    G4EmParameters* param = G4EmParameters::Instance();
