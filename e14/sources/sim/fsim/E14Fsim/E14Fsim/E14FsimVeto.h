...
class E14FsimVeto {
 public:
  ...
  // method
  void  setDetID(int idetid) { m_detid = idetid; }
  void  setPID(int ipid) { m_pid = ipid; }
  void  setEnergy( double energy );
  void  setTime( double time );
  void  setIneffi( double ineffiv ) { m_ineffi = ineffiv; }
  void  setPos( const TVector3& posv ){ m_pos = posv; };
  void  setPos( double xx, double yy, double zz ) { setPos(TVector3( xx,yy,zz )); }
  void  setMome( const TVector3& momev ) { m_mome = momev; }
