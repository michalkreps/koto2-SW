...
\maketitle
 ...
 The gsim4 is a framework library for a interface of Geant4 with 
 ROOT. The particle transportation and detector simulation are performed
 with Geant4 and the data is stored with ROOT framework.
 ...
 An example program using the gsim4 framework is ``gsim4test'',
...
(loop counter) is defined and used inside.
 ...
 radian (1.0),
 rad (1.0),
 degree (3.14159265358979323846/180.), and
 ...
 \begin{table}[H]
  ...
  \begin{tabular}{l|rl}
   ...
   KLbeta&1000130 &$K_L\to K^- e+\nu_e$\\
   KLpi02gamma&2000130 &$K_L\to\pi^0\gamma\gamma$\\
   KLpi0pi0nunu&11000130 &$K_L\to\pi^0\pi^0\nu\bar\nu$\\
   KL2pi0gamma&12000130 &$K_L\to\pi^0\pi^0\gamma$\\
   KLpi0pi0Agamma&13000130 &$K_L\to\pi^0\pi^0A\gamma$\\
   KLpi0gamma&14000130 &$K_L\to\pi^0\gamma$\\
   KLpipigamma&15000130 &$K_L\to\pi^+\pi^-\gamma$\\
   KLeegamma&16000130 &$K_L\to e^+e^-\gamma$\\
   KLmumugamma&17000130 &$K_L\to\mu^+\mu^-\gamma$ \\
   KLpi0gammaA&18000130 &$K_L\to\pi^0\gamma A$ \\
   KLpipi0enu&19000130 &$K_L\to\pi^\pm pi^0 e^\mp\nu$
  \end{tabular}
 \end{table}
...
\subsubsection{Clone}
All the physical detector is created with G4PVPlacement.
``Clone'' method is implemented to make another G4PVPlacement
with the same G4Logical volume, which means also the same G4Solid.
